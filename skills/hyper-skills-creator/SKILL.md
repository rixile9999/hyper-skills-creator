...
- **`personalizing-skills`** — tune a chosen skill → verify.
...
```
intent ─▶ finding-skills ─(chosen base, wants tuning)─▶ personalizing-skills ─▶ done
            │ adopt as-is                                      │
            └────────────────── done                          verify ─▶ done
```
...
| already has a skill, wants it adapted | **`personalizing-skills`** |
...
1. **Discovery** — invoke `finding-skills`. It clarifies intent, searches the
   ...
   (adopt / personalize / keep looking).
2. **The chain point** — when `finding-skills` lands on *personalize* (good base,
   wrong defaults), the journey continues into `personalizing-skills`. Carry the
...
3. **Tuning** — `personalizing-skills` captures preferences, picks a mode
...
"help me with skills" (vague)
   ...
   route (Step 1)
        ...
        ├── tune existing ────────────────────────────────┤
        │                                                  ▼
        └── both ──▶ finding-skills ──▶ personalizing-skills ──▶ verify ──▶ done
```
...
- **Dropping context at the chain point.** When `finding-skills` → `personalizing-skills`,
...
- **Auto-picking a path.** Whether to adopt-as-is, personalize, or keep looking
